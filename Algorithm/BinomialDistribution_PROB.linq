...
{
	int faces = 10;
	var printTableProbability = true;
	
	int facesPlus = faces + 1;
	double right = facesPlus / 1.618;
	double left = (facesPlus / 2) - (right - (facesPlus / 2));
	double rightInt = (int)Math.Round(right);
	double leftInt = (int)Math.Round(left);
	var results = new List<BigDecimal>();

	for (int i = 0; i <= faces; i++)
	{
		var fatorLeft = Fatorial(faces);
		var fatorRight = BigInteger.Multiply(Fatorial(i), Fatorial(faces - i));
		...
		if (i != faces)
			powRight = BigDecimal.Pow(new BigDecimal(5, 1, 1000000000), (faces - i));
		var prob = new BigDecimal(fat) * powLeft * powRight;
		results.Add(prob);
	}
	var resultCount = results.Count;
	int middleLeft = resultCount / 2;
	int middleRight = middleLeft * 2 < resultCount ? middleLeft + 1 : middleLeft;
	...
	if (!printTableProbability)
	{
		...
		while ((sum * new BigDecimal(100, 0, 1000000000)) < new BigDecimal(9999, 2, 1000000000))
		{
			...
			if (middleRight <= faces)
				sum = sum + results[middleRight];
		}

	(sum * new BigDecimal(100, 0, 1000000000)).Dump("sum");
		middleLeft.Dump("middleLeft");
		middleRight.Dump("middleRight");
		(middleRight - middleLeft).Dump("itens of sum");
		((middleRight - middleLeft) * 100 / resultCount).Dump("percent of sum");
