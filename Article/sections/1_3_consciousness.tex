...
Na Figura \ref{fig:consciousness_space_3D_plan} é possível observar as posições dos pontos e as proporções que cada subintervalo ocupa dentro da população. No subintervalo 1 não é possível observar seus subintervalos, então o ponto representado pela letra A refere-se às quantidades de amostras desse subintervalo em cada lado do par de amostras (os eixos X e Y, por exemplo) e centrados no eixo Z, proporção desse intervalo dentro da população. As sombras no plano podem ajudar na observação de posições e proporções. Nos subintervalos 2 e 5, seus subintervalos podem ser observados, e o ponto do intervalo superior não é mais observado. Os cubos transparentes representam a proporção do intervalo ou subintervalo dentro da população, com todos os cubos representando o mesmo número de pontos, mas em escalas diferentes. Na Figura \ref{fig:consciousness_space_3D_plan_plus} um novo subintervalo é representado (6) tornando todos os outros menores dentro da população proporcionalmente. É importante notar que os pontos são a parte visível e os cubos estão nas figuras apenas para facilitar o entendimento.

Outro fato importante nas Figuras \ref{fig:consciousness_space_3D_plan} e \ref{fig:consciousness_space_3D_plan_plus} é a relação das proporções dos subintervalos que diminuem à medida que a distância entre os pontos dos intervalos aumentam, uma vez que novas amostras são adicionadas a eles. Isso é consistente com o átomo, onde o núcleo está proporcionalmente distante dos elétrons, e com a inflação cósmica, por exemplo.
	\begin{figure}[H]
	\centering
