...
O histograma é utilizado nas figuras dessa subseção e posteriormente para facilitar a visualização e entendimento, pois representa muito bem a curva de densidade de uma população, conforme as diferentes visualizações da Figura \ref{fig:consciousness_wave_histogram}, a qual representa apenas um intervalo ou um comprimento de onda pareado pela mediana da população.  
	\begin{figure}[H]
...
Em intervalos menores e com muitos momentos lógicos é observado uma discrepância maior das amplitudes das ondas. Nesses intervalos podem ser observados sistemas menores de objetos. Quanto menores os intervalos mais desequilibrados eles estarão crescendo sentido a mediana da população, probabilisticamente, conforme Figura \ref{fig:consciousness_space_subconsciousness_min}. A onda mais inferior, azul escuro, é a onda base do sistema, ou seja, a onda formadora de outras ondas. Os sistemas de ondas mais complexos e com essa característica podem representar, por exemplo, o átomo que são muito pequenos, se apresentam em enormes quantidades e as partículas que orbitam seu núcleo (elétrons) ficam bem mais distantes dele.
	...
	\centering
	\includegraphics[scale=.4]{sections/images/consciousness_space_subconsciousness_min.jpg}
...
Na Figura \ref{fig:consciousness_space_subconscious_observation_jump} é observado os entrelaçamento de ondas (representadas por colunas de um histograma para facilitar a visualização do intervalo). A reordenação feita pelo entrelaçamento provoca um salto nas coordenadas (X, Y e Z) conforme subseção do Espaço.
	...
	\centering
	\includegraphics[scale=.65]{sections/images/consciousness_space_subconscious_observation_jump.jpg}
...
Probabilisticamente, a grande concentração das amostras de uma população está em seu pico, sentido a mediana da população. Assim, devido à alta concentração probabilísticas de amostras em intervalos cada vez menores de uma onda, o pico irá ocupar um subintervalo proporcional cada vez menor dentro da população, conforme observado na Figura \ref{fig:consciousness_flat_universe}. A Figura \ref{fig:total_comparison_chart_with_99_range} que é fundamentada na Tabela \ref{tab:10000_all} também demostra essa características. Essa característica dentro da população demostrar um universo aproximadamente plano em sua distribuição.  
	\begin{figure}[H]
	\caption{Universo plano}
	\label{fig:consciousness_flat_universe}
	\centering
	\includegraphics[scale=.7]{sections/images/consciousness_flat_universe.jpg}
	\floatfoot{Concentração dos 99\% das amostras mais relevantes.}%\footnotemark}
	\end{figure}
	%\footnotetext{Fonte: note}

Na Figura \ref{fig:consciousness_space_system} é possível o observar que intervalos entrelaçados menores são dispostos em intervalos entrelaçados maiores. A coordenada (z) desses subintervalos são ligeiramente diferentes, porém como descrito no parágrafo anterior, elas estão tão próximas que sua representação como uma única coluna do histograma é válida. 

Os colchetes na Figura abaixo representam um range observável (o observador é melhor descrito a subseção da consciência, Observador e a vida). E dentro do range do intervalo maior estão representados três subintervalos entrelaçados, os quais podem ter suas coordenadas comparadas em termos de comprimento (z) e amplitudes (x  e y). O comprimento é relativo ao tamanho do intervalo e representa a área cubica e A posição dentro da população. A amplitude dos subintervalos dentro de um intervalo é relativa à sua densidade de amostras, portanto, com base na densidade do subintervalo representado na Figura abaixo ele é posicionado no intervalo maior que o contém.

Os subintervalos que estão no topo de um intervalo são os que tem a maior densidade de amostras, mesmo que no interior desses subintervalos hajam ainda subintervalos com densidades bem baixas.

Ao observar apenas um range de um subintervalo, como o da Figura abaixo, não é possível saber sua posição no intervalo maior que o contém. 
	\begin{figure}[H]
	\caption{Intervalo e subintervalo}
	\label{fig:consciousness_space_system}
	\centering
	\includegraphics[scale=.7]{sections/images/consciousness_space_system.jpg}
	\floatfoot{Representação de intervalos e subintervalos nas coordenadas espaciais.}%\footnotemark}
	\end{figure}
	%\footnotetext{Fonte: note}
...
Quando um intervalo se aproxima de outro, seus pares de ondas ficam cada vez mais parecidos (eixos X e Y) devido as probabilidades das distribuições dessas novas amostras em intervalos cada vez mais próximos na população. A aproximação faz com que os pares de ondas de um objeto se pareça muito com os pares de ondas do outro objeto quando esses possuem baixa entropia. Devido a aproximação de dois objetos que possuem baixa entropia, o intervalo que os separam, probabilisticamente, também recebera amostras com essas características, o que torna muito de seus pares viáveis para que o entrelaçamento de ondas encontre pares mais ideais nesse intervalo ou no outro objeto e vice-versa. Desta forma, ocorre uma reordenação entre os dois subconjuntos e o intervalo que os separam, diminuindo esse intervalo por meio do entrelaçamento de ondas. Essa reordenação torna esse intervalo (subconjuntos e o intervalo reduzido entre eles) mais equalizado (baixa entropia).
...
Ao adicionar uma nova amostra todo o intervalo se distribui proporcionalmente para acoplar essa amostra, conforme observado na \ref{fig:consciousness_dark_matter_dark_energy}. Logo, o afastamento dos intervalos entrelaçados de uma população ocorre devido a densidade que não param de crescer dentro desses intervalos. Portanto, o efeito da gravitação e da expansão do universo estão ligadas a probabilidade maior de distribuição de novas amostras no pico de um intervalo, seja ele qual for. A expansão do universo representa o maior pico da população, onde, probabilisticamente, tem a maior concentração de amostra e, portanto, a maior gravidade, conforme Figura \ref{fig:consciousness_space_system}. 
	\begin{figure}[H]
	\caption{Aspectos probabilísticos da matéria e energia escuras}
	...
	\floatfoot{A matéria e energia escuras são apenas aspectos probabilísticos.}%\footnotemark}
	\end{figure}
	%\footnotetext{Fonte: note}
...
Devido à alta concentração probabilísticas de amostras em intervalos cada vez menores de uma onda, o pico visível irá ocupar um subintervalo proporcional cada vez menor dentro do intervalo da onda, mesmo com uma concentração de amostras crescentes, conforme observado na Figura \ref{fig:consciousness_dark_matter_dark_energy}. Esses picos são frequentemente encontrados da mediana para frente dos sistemas (o núcleo ou pico do sistema).
