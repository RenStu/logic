...
In Figure \ref{fig:consciousness_space_3D_plan} it is possible to observe the positions of the points and the proportions that each subinterval occupies within the population. The Z-axis represents the subintervals, which are cubes when they do not contain subintervals, representing the proportional area within the population in 1 dimension. However, these cubes can be represented by rectangles when they have subintervals, according to the subintervals (2) of Figure \ref{fig:consciousness_space_3D_plan_subinterval}. Each cube or rectangle represents the same number of population points (such as between 0 and 1), but on a different scale, depending on their proportion in the population in 1 dimension. The points inside the cubes or rectangles are the actually observable parts (the transparent cubes and rectangles are to help you understand the positions and proportions of the subintervals) and the size of each point depends on the density of each subinterval. The subinterval (1) of Figure \ref{fig:consciousness_space_3D_plan_subinterval} has no subintervals, so the point represented by the letter A refers to the quantities of samples of that subinterval on each side of the pair of samples (the X and Y axes, for example ) and centered on the Z-axis. The shadows on the plane in Figure \ref{fig:consciousness_space_3D_plan_subinterval} can also help in observing the positions and proportions.
	\begin{figure}[H]
	\centering
	...
	\floatfoot{Comparison of the positions and proportions of subintervals in a population. \protect\footnotemark}
	\end{figure}
	\footnotetext{Figures can be accessed in 3 dimensions at: \url{https://www.tinkercad.com/things/6RvwcZUydui} and \url{https://www.tinkercad.com/things/ihCGf5VPy6N}}

The subinterval (2) in Figure \ref{fig:consciousness_space_3D_plan_subinterval} has subintervals, so if observer is able to observe at that lower level, the point representing the higher level is no longer observable. Since the Z-axis represents the interval or subintervals, it is important to note that the Z-axis of subinterval (2) is perpendicular to the Z-axis of its upper subinterval. Otherwise the subintervals would divide a larger Z of the upper interval into smaller Zs. Subintervals will always have their Z-axis perpendicular to the Z-axis of their upper interval, guaranteed that they are within a single upper interval, without subdividing it into Zs (which would represent other subintervals and not the upper interval).

In Figure \ref{fig:consciousness_space_3D_plan_subinterval} a new subinterval is represented (6) making all others smaller within the population proportionally. The distribution of the subintervals also decreases their upper interval according to the discrepancy of their distributions, consistent with the subinterval (2). But the population size in 1 dimension never changes, the organizations of the subintervals cause changes only in 2 or 3 dimensions due to the nested groupings. The relationship of the proportions of the subintervals decreasing as the distances between the points in the intervals increase proportionally (when new samples are added to them) is consistent with the atom, where the nucleus is proportionally distant from the electrons, and also with cosmic inflation, for example.

In Figure \ref{fig:consciousness_space_3D_plan_subinterval_amplify} subinterval A (edge of the wave) and subinterval B (core of the wave) are compared, where point A tends to be less dense and therefore tends to have fewer subintervals than point B, which tends to be denser. The more subdivided an interval is, the more discrepant its distribution tends to be, as shown in Figures \ref{fig:consciousness_space_subconsciousness} and \ref{fig:consciousness_space_subconsciousness_min} where larger intervals are less discrepant than smaller ones. Thus, subinterval B tends to decrease more slowly than A, but A tends to have fewer nested subintervals, which causes it to stop decreasing while B tends to continue. This relationship can cause the figure effect \ref{fig:consciousness_space_3D_plan_subinterval_amplify} or \ref{fig:consciousness_space_3D_plan_subinterval_contract}.
	\begin{figure}[H]
	\centering
		\begin{subfigure}[H]{0.65\linewidth}
		\centering
	...
	\floatfoot{Comparison of subinterval contractions.}
	\end{figure}
...
The universe has no defined shape and the observer, represented in Figure \ref{fig:consciousness_amplitude_viewpoint} by the human being, combines his wavelengths with the wavelengths obtained by the senses, observing the shapes of the universe in his own way. Obtaining samples through the senses modifies them and, in this way, these ripples act as adjustments or configurations. Each sense observes the sample population independently, as distinct frequency channels. Thus, vision may be focusing on very distant objects and ears focusing on very close sounds.
...
Also in Figure \ref{fig:consciousness_amplitude_viewpoint} it can be seen that the wider the objects observed (more columns of the histogram) at small intervals (viewpoint A - top of the histogram columns in purple), they can still be observed at larger intervals (viewpoint B). This is how a galaxy can turn into a point when viewed at very large wavelengths or amplitudes.
	...
	\floatfoot{The cognitive part of the human subconscious and its independent observations through the senses.}%\footnotemark}
	\end{figure}
	%\footnotetext{Fonte: note}}
...
In Figure \ref{fig:consciousness_amplitude_crest_valley} an analogy is made of the light blue dashed line with the logical wave of planet Earth, for example. The crest of the wave is the part that receives the most samples, therefore the hot and bright part that comes from sunlight (day). This wave can represent the rotational movement of the Earth, and when the human wave is in the valley of the planet wave, it is when it receives the least amount of samples (night), so the senses tend to receive less stimuli and fall asleep more easily, it is the falling asleep of the human subconscious.
	...
	\floatfoot{Earth crest and valley as a characteristic of the numbness of the human senses.}%\footnotemark}
	\end{figure}
	%\footnotetext{Fonte: note}}
...
The Figure below also contemplates the interval observed as a wave, where the observer fixates on an interval represented by one of the parts that make up the entangled pair and follows its motions and jumps, since the jumps are frequent in small intervals.
	\begin{figure}[H]
	...
	\floatfoot{Characteristics of observing a small part of a subset.}%\footnotemark}
	\end{figure}
	%\footnotetext{Fonte: note}}
